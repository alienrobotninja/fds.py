...
def test_public_key_derivation(crypto_instance):
    ...
    # crypto = Crypto()
    generated_public_key1 = crypto_instance.private_to_public_key(private_key1)
    generated_public_key2 = crypto_instance.private_to_public_key(private_key2)

    assert generated_public_key1 == public_key1
    assert generated_public_key2 == public_key2


def test_calculate_shared_secret(crypto_instance):
    # crypto = Crypto()
...
def test_encrypt_and_decrypt_string(crypto_instance):
    # crypto = Crypto()
    string_to_encrypt = "Hello, World!"
    password = b"supersecretpass"  # Should be bytes
    iv = crypto_instance.generate_random_iv()

    encrypted_string = crypto_instance.encrypt_string(string_to_encrypt, password, iv)
    decrypted_string = crypto_instance.decrypt_string(encrypted_string, password, iv)
